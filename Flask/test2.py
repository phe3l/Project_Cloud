...
def get_forecast(ip):
    try:
        response = urequests.post("http://130.223.238.17:8080/future-weather", json={"ip": ip})
        if response.status_code == 200:
            forecast_data = response.json()
            display_forecast(forecast_data.get('data'))
            outdoor_temp = forecast_data['list'][0]['main']['temp'] if 'list' in forecast_data and forecast_data['list'] else None
            return outdoor_temp  # Return the outdoor temperature for further use
        else:
...
def send_data(ip, outdoor_temp):
    ...
    try:
        response = urequests.post("http://130.223.238.17:8080/send-to-bigquery", json=data)
    except Exception as e:
...
def main_loop():
    ...
    while True:
        ...
        if time.ticks_diff(time.ticks_ms(), last_data_sent_time) >= 60000:
            ...
            if public_ip:
                outdoor_temp = get_forecast(public_ip)  # This will also update the forecast on display
                if outdoor_temp is not None:
                    send_data(public_ip, outdoor_temp)
            last_data_sent_time = time.ticks_ms()
